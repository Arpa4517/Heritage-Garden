...
export const getUser = async (req, res) => {
  try {
      ...
      const user = await User.findById(userId).select('-password'); // Fetch user data except password
      if (!user) {
      ...
      return res.status(500).json({ message: 'Error retrieving user data: ' + error.message });
  }
};
